const API = '';
const SHOP_CONFIG = window.SHOP_CONFIG || { slug: 'demo', name: 'Aroma Match' };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.topbar-logo').forEach(el => { el.href = SHOP_CONFIG.home_url || '/'; });
});
...
async function submitQuiz() {
  ...
  try {
    const res = await fetch(`${API}/api/recommend/quiz/${SHOP_CONFIG.slug}`, {
      method: 'POST',
...
function addUTM(url) {
  ...
  return url + sep + `utm_source=aroma-match&utm_medium=quiz&utm_campaign=${SHOP_CONFIG.slug}`;
}
...
function restart() {
  window.location.href = '/quiz/' + SHOP_CONFIG.slug;
}
